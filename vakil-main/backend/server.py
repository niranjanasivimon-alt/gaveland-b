...
# MongoDB connection
mongo_url = "mongodb+srv://vakiluser:vakil123@cluster0.yewv3qw.mongodb.net/vakilsetu_db?retryWrites=true&w=majority"

from motor.motor_asyncio import AsyncIOMotorClient

client = AsyncIOMotorClient(mongo_url)
db = client["vakilsetu_db"]
...
async def get_current_user(request: Request) -> dict:
    ...
    try:
        ...
        if user:
            ...
            return user
        lw = await db.legal_writers.find_one({"_id": uid})
        if lw:
            lw["id"] = str(lw["_id"])
            lw.pop("_id", None)
...
async def accept_case(case_id: str, current_user: dict = Depends(get_current_user)):
    ...
    try:
        result = await db.cases.update_one(
            {"_id": ObjectId(case_id), "status": "open"},
            {"$set": {"status": "accepted", "lawyer_id": current_user["id"]}}
        )
...
async def startup_event():
    ...
    await db.cases.create_index("lawyer_id", sparse=True)
    await db.cases.create_index("nyayId", unique=True, sparse=True)
    await db.users.create_index("firmId", sparse=True)
    await db.legal_writers.create_index("email", unique=True)
    await db.drafts.create_index("status")
    await db.drafts.create_index("legal_writer_id", sparse=True)
    ...
    if firms_count == 0:
        ...
        # Link demo lawyers to firms by email
        firm_lawyer_map = {
        ...
        # Ensure lawyer firm_name field is populated if missing
    ...
    # Seed demo cases (only if the test client has none) so the demo lawyer
    # immediately sees content in "Available Cases" and "My Active Cases".
    ...
    if test_client and test_lawyer:
        ...
        if existing_demo_cases == 0:
            ...
            demo_cases = [
                ...
                # Two cases ALREADY assigned to the demo lawyer so the
                # "My Active Cases" tab is populated on first login.
                {
                    "case_type": "Criminal",
    ...
    with open(memory_path / "test_credentials.md", "w") as f:
        ...
        f.write("## Test Client\n")
        f.write(f"Email: {test_client_email}\n")
