...
const MyCases = () => {
  ...
  // Live-poll status for all cases every 12s
  useEffect(() => {
    ...
    const pollStatuses = async () => {
      ...
      results.forEach((r, i) => {
        if (r.status === 'fulfilled') {
          ...
          updates[cid] = d;
          // detect a real status change from previous poll
          if (prevStatusRef.current[cid] && prevStatusRef.current[cid] !== d.case_status) {
            flashes[cid] = true;
          }
          prevStatusRef.current[cid] = d.case_status;
        }
      });
    ...
    pollStatuses();
    const iv = setInterval(pollStatuses, 12000);
    return () => clearInterval(iv);
  }, [cases]);
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1 flex items-center gap-2">
              Track your legal cases in real time
              <LiveDot />
            </p>
          </div>
